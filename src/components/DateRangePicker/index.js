...
function DateRangePicker(props) {
  ...
  // Memoized function to update selected date text
  const updateSelectedDateText = useCallback(range => {
    try {
  ...
  }, [props.ranges, updateSelectedDateText]);
  ...
  const handleRangeChange = useCallback(
    ...
    [props.onChange, updateSelectedDateText]
  );
  ...
  const handleCancel = useCallback(() => {
    ...
    if (props.onChange) {
      props.onChange({ selection: defaultRange });
    }
  }, [props.onChange]);
    ...
    <div
      ...
      <div
        style={{
          position: 'relative',
          // width: 'fit-content',
          minWidth: `${!customInput ? '300px' : 'auto'}`,
          // overflow: 'hidden',
        }}>
        {renderInput()}
        {!customInput && (
      ...
      {showDropdown && (
        <div
          ...
          style={{
            ...
            zIndex: 9999,
            borderRadius: '4px',
            minWidth: `${Math.max(300, months * 300)}px`,
          }}
          {...attributes.popper}>
          <div>
            <DefinedRange
